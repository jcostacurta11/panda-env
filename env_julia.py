...
class SimpleEnv():
    ...
    def __init__(self):
        ...
        p.loadURDF(os.path.join(self.urdfRootPath, "table/table.urdf"), basePosition=[0.5, 0.3, -0.65])

        # example YCB object
        obj1 = YCBObject('011_banana')
        obj1.load()
        p.resetBasePositionAndOrientation(obj1.body_id, [0.7, 0, 0.1], [0, 0, 0, 1])
    ...
    def _set_camera(self):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=0, cameraPitch=-10,
    ...
    def grip(self, action):
        # action in this example is the
        self.panda.step(grasp_open=action)
    ...
    def position(self, action,grasp,vel, move):
        # action in this example is the
        self.panda.position(dposition = action, grasp_open=grasp, v = vel, move = move)
